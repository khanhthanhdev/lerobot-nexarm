...
void SerialServo_t::word2bytes(uint16_t word, uint8_t *bytes_l, uint8_t *bytes_h)
{
    if (endianness) {
	    *bytes_l = (word >> 8);
		*bytes_h = (word & 0xff);
...
uint16_t SerialServo_t::bytes2word(uint8_t *bytes_l, uint8_t *bytes_h)
{
    uint16_t word;
...
ServoStatus_t SerialServo_t::ack()
{
...
void SerialServo_t::begin(HardwareSerial& uart, uint32_t baudrate, uint8_t tx_pin, uint8_t rx_pin) {
	this->uart = &uart;  // 正确：将传入的串口引用地址赋值给成员变量
	this->uart->setTxBufferSize(512);
	this->uart->setRxBufferSize(512);
	this->uart->begin(baudrate, SERIAL_8N1, rx_pin, tx_pin);  // 使用tx和rx引脚参数初始化串口
	rx_skip = 1;
	rx_timeout = 20;
	endianness = 0;
}
...
ServoStatus_t SerialServo_t::general_write(uint8_t id, uint8_t addr, uint8_t *data, uint8_t data_len)
{
	ServoStatus_t status;
...
ServoStatus_t SerialServo_t::general_read(uint8_t id, uint8_t addr, uint8_t *data, uint8_t data_len)
{
	ServoStatus_t status;
	uint8_t buf[2];
...
ServoStatus_t SerialServo_t::sync_write(uint8_t addr, uint8_t *data, uint8_t data_len, uint8_t parameter_len)
{
	ServoStatus_t status;
...
ServoStatus_t SerialServo_t::enable_torque(uint8_t id)
{
	uint8_t data = 1;
...
ServoStatus_t SerialServo_t::disable_torque(uint8_t id)
{
	uint8_t data = 0;
...
ServoStatus_t SerialServo_t::cali_pos(uint8_t id)
{
	uint8_t data = 128;
...
ServoStatus_t SerialServo_t::write_pos_offset(uint8_t id, int16_t offset)
{
	uint16_t data;
...
ServoStatus_t SerialServo_t::write_pos_ex(uint8_t id, uint8_t acc, int16_t speed, int16_t pos)
{
	uint8_t data[7];
	uint8_t	_acc;
	uint16_t _pos;
...
ServoStatus_t SerialServo_t::write_reg_pos_ex(uint8_t id, uint8_t acc, int16_t speed, int16_t pos)
{
	uint8_t data[7];
	uint8_t	_acc;
	uint16_t _pos;
...
ServoStatus_t SerialServo_t::sync_write_pos_ex(int16_t (*data)[4], uint8_t id_num)
{
	ServoStatus_t status;
...
ServoStatus_t SerialServo_t::sync_read_cur_pos_ex(uint8_t *id, uint8_t id_num, int16_t (*data)[5])
{
	ServoStatus_t status;
	uint8_t byte_len = 8;
...
ServoStatus_t SerialServo_t::read_pos_offset(uint8_t id, int16_t *offset)
{
	ServoStatus_t status;
	uint8_t data[2];
	uint16_t _offset;
...
ServoStatus_t SerialServo_t::read_pos(uint8_t id, int16_t *pos)
{
	ServoStatus_t status;
	uint8_t data[2];
	uint16_t _pos;
                                            ...
                                            uint16_t *cur)
{
	...
	_status = general_read(id, REG_PRESENT_POSITION_L, data, 13);
	if(_status.error_bits.bit_tx || _status.error_bits.bit_rx) {
		*pos = 0;
		*speed = 0;
		return _status;
	}
	...
	*speed = (int16_t)MASK_SERVO(_speed, 15);
	...
	_load = bytes2word(data + 4, data + 5);
	*load = (int16_t)MASK_SERVO(_load, 10);

    *vol = data[6];
    *temp = data[7];
    *status = data[9];
	_cur = bytes2word(data + 10, data + 11);
	*cur = _cur;
